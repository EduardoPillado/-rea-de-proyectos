            ...
            <div style="padding-bottom: 4%" class="table-back">
                            ...
                            <table class="table table-bordered border-list" id="tabla-completa">
                                ...
                                @foreach ($dato->productos as $producto)
                                    <tr>
                                        ...
                                        <td>
                                            @foreach ($dato->entrada_producto as $ent_prod)
                                                @if ($ent_prod->fk_producto == $producto->pk_producto)
                                                    {{$ent_prod->cant_unidades}}
                                                @endif
                                            @endforeach
                                        </td>
                                        ...
                                        <td>
                                            @foreach ($dato->entrada_producto as $ent_prod)
                                                @if ($ent_prod->fk_producto == $producto->pk_producto)
                                                    @if ($ent_prod->importe_dls !== null)
                                                        ${{$ent_prod->importe_dls}}
                                                    @else
                                                        $0.00
                                                    @endif
                                                @endif
                ...
                <div style="text-align: center">
                    @if ($dato->estatus == 'Activo')
                        <a href="{{route('entrada.bloquear', $dato->pk_entrada)}}" title="Bloquear" class="filtrados" id="filtrados">
                            <i style="color: white" class="bi bi-lock"></i>
                        </a>
                    @else
                        <a href="{{route('entrada.activar', $dato->pk_entrada)}}" title="Activar" class="filtrados" id="filtrados">
